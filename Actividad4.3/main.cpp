...
class Sorter {
public:
    // Comparar si la cantidad de strings de un vector en la lista se repite más que el del otro
...
void displayPorts(vector<auto> v) {
    ...
    PuertosPeligrosos.open("output/Puertos mas peligrosos.txt");
    PuertosPeligrosos << "Los top 10 puertos\n";
    for (int i = 0; i < v.size(); i++) {
        PuertosPeligrosos << i << ".- " << v[i] << endl;
    }
...
void displayGraph(vector<string> adj[], vector<string> IPs, vector<string> res, int V) {
    /*Se navega la lista de los dos vectores, donde n es tantola posición del IP
    y la posición del arreglo de los puertos del mismo IP. Se compara primero la lista de IPs
    con la lista de IP ordenada por la cantidad de intentos de acceso al mismo.*/
    ofstream ListaAdyacencia;
    ListaAdyacencia.open("output/Lista de adyacencia y Top 10.txt");
    ListaAdyacencia << "Adyacencia de IPs (10 IPs mas vulnerables)\n";
    for (int i = 0; i < res.size(); i++) {
        for (int n = 0; n < V; ++n) {
            if (IPs[n] == res[i]) {
                ListaAdyacencia << "{" << IPs[n] << "}";
                for (auto x : adj[n]) {
                    ListaAdyacencia << " -> " << x;
                }
                ListaAdyacencia << "\n";
...
// Busca la posición de cada ip en su respectivo vector para adherirlo a la lista de puertos en su posicion correcta
int posicionDuplicados(vector<string> comparative, string ip) {
    int a;
    for(a = 0; a < comparative.size(); a++) {
        // Si el IP ya existe en un punto de la lista, se regresa la posición del IP ya en la lista
        if (comparative[a] == ip){
            return a;
        }
    }
...
int main() {
    ...
    string sline;
    // Archivo de los registros
    ifstream bitacoraNo2("input/bitacora3.txt");
    while(getline(bitacoraNo2, sline)) {
        istringstream iss(sline);
        string s = sline;
        s = s.replace(0,4,mesTextoANro(sline.substr (0,4))).replace(0,1,"T");
        stringstream ss(s);
        istream_iterator<string> begin(ss);
        istream_iterator<string> end;
        vector<string> vstrings(begin, end);
    ...
    ListaAdyacencia.close();
    cout << endl;
    cout << "Puede encontrar la lista de adyacencia en el archivo Lista de adyacencia y Top 10.txt\n";
    ...
    ListaAdyacencia.close();
    cout << endl;
    cout << "Puede encontrar la lista de puertos en el archivo Puertos mas peligrosos.txt\n";
    cout << endl;
